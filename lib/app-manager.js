...
export default function initAppManager(w: Window) {
  return class AppManager {
    ...
    _status: $Values<typeof AppManager.statuses> = this.constructor.statuses.DEFAULT;
    ...
    _unmount = (appName: AppNameType, eventTitle: string) => async (resolvedFragment: ResolvedFragmentType): Promise<?boolean> => {
      const { script, name, slot } = resolvedFragment;
    ...
    _mount = (appName: AppNameType, eventTitle: string) => async (resolvedFragment: ResolvedFragmentType): Promise<boolean> => {
      const { script, name, slot } = resolvedFragment;
    ...
    _onStateChange = (appName: AppNameType, eventTitle: string) => async (resolvedFragment: ResolvedFragmentType): Promise<boolean> => {
    ...
    _onUpdateStatus = (appName: AppNameType, newStatus: string) => async (resolvedFragment: ResolvedFragmentType): Promise<boolean> => {
      const { script, name } = resolvedFragment;
      ...
      try {
        ...
        await script.onUpdateStatus(newStatus, app);
    ...
    _updatePage = async (newAppName: AppNameType, eventTitle: string): Promise<boolean> => {
