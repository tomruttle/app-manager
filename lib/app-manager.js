...
export default function initAppManager(w: WindowType, html5History: History) {
  ...
  return class AppManager {
    ...
    static triggerEvent(eventTitle: $Values<typeof AppManager.eventTitles>, data: mixed): void {
      history.events.trigger(eventTitle, data);
    }

    static bindEvent(eventTitle: $Values<typeof AppManager.eventTitles>, callback: (data: mixed) => mixed): void {
      history.events.bind(eventTitle, callback);
    }

    static unbindEvent(eventTitle: $Values<typeof AppManager.eventTitles>): void {
      history.events.unbind(eventTitle);
    }
