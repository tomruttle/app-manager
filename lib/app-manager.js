...
export default function appManager(config: ConfigType, options: OptionsType) {
  ...
  async function getFragmentScript(fragmentName: FragmentNameType, loadScript: LoadScriptType): Promise<LoadedScriptType> {
    if (cachedScripts[fragmentName]) {
      return cachedScripts[fragmentName];
    }
  ...
  async function* createLifecycle(slotName: SlotNameType, fragmentName: FragmentNameType, onError: (err: Error) => void, initialRender: boolean): LifecycleType {
    ...
    if (!script) {
      if (!initialRender) {
        ...
        if (slot) {
          ...
          if (typeof getErrorMarkup === 'string') {
            try {
  ...
  async function stateChanger(browserState: BrowserStateType, onRecoverableError?: OnErrorCallbackType): Promise<boolean> {
    ...
    const mountPromises = () => slotsToMount.map(async (slotName) => {
      ...
      try {
        mountedLifecycles[slotName] = await createLifecycle(slotName, fragmentName, onError, initialRender);
        const { value } = await mountedLifecycles[slotName].next(false);
        return value;
      } catch (err) {
        await handleLifecycleError(slotName, err);
