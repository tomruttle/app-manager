...
export default function appManager(config: ConfigType, options: OptionsType) {
  ...
  const loadWrappedScript = initLoadWrappedScript(fragments, loadScript, options);
  ...
  // TODO: Workaround until https://github.com/facebook/flow/issues/285 is fixed.
  const attributes = {
    writable: false,
    enumerable: true,
    get() { return state; },
  ...
  // $FlowFixMe
  Object.defineProperty(stateChanger, 'state', attributes);
