  ...
  return class AppManager implements AppManagerInterface {
    ...
    _updatePage = async (path: string, search: string, hydrate: boolean, eventTitle?: mixed): Promise<void> => {
      ...
      if (hydrate) {
        ...
        const slotsToUpdate = oldAppSlots.filter((slotName) => {
        ...
        const slotsToUnmount = oldAppSlots.filter((slotName) => {
          const fragmentName = oldAppFragments[slotName];
          return (
            fragmentName
            && this._mountedLifecycles[slotName]
    ...
    _runStateChanger = (hydrate: boolean, eventTitle?: mixed): void => {
    ...
    _handleStateChange = (eventTitle: mixed): void => {
      this._runStateChanger(false, eventTitle);
    }

    init = (): void => {
      this._shimHistory();
      this._runStateChanger(true);
    }
  };
}
