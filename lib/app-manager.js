...
import { defaultGetRouteName, defaultGetElement, setNames } from './utils/config';
...
export function getAppManagerScript(config: ConfigType, events?: ?EventsType, options?: ?OptionsType): AppManagerScriptType {
  ...
  let getRouteName = defaultGetRouteName(routes);
  ...
  if (options) {
    ...
    if (typeof options.getRouteName === 'function') {
  ...
  const updateState = initUpdateState(routes, getRouteName, getAdditionalState);
