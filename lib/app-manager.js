...
export default function initAppManager(w: Window) {
  return class AppManager {
    ...
    _unmount = (oldAppName: AppNameType, newAppName: AppNameType, eventTitle: EventTitleType) => async (resolvedFragment: ResolvedFragmentType): Promise<?boolean> => {
      const { script, name, slot: slotName } = resolvedFragment;
      ...
      try {
        ...
        const app = this._apps[oldAppName];
    ...
    _hydratePage = async (currentAppName: AppNameType): Promise<boolean> => {
      try {
    ...
    _updatePage = async (newAppName: AppNameType, eventTitle: EventTitleType): Promise<boolean> => {
      ...
      const unmountPromises = oldResolvedFragments
        ...
        .map(this._unmount(oldAppName, newAppName, eventTitle));
    ...
    init = async (): Promise<boolean> => {
      ...
      const hydrateSuccess = await this._hydratePage(currentAppName);
