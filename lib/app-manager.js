  ...
  return class AppManager {
    ...
    _resolveFragment = async (slotName: SlotNameType, fragmentName: FragmentNameType, state: StateType): Promise<?ResolvedFragmentType> => {
      ...
      if (fragment && fragment.loadScript) {
        ...
        resolvedFragment = { script, slot: slotName, name: fragmentName };
      } else {
    ...
    _resolveAppFragments = async (appName: AppNameType, state: StateType): Promise<Array<?ResolvedFragmentType>> => {
    ...
    _handleStateChange = async (eventTitle: mixed): Promise<void> => {
      ...
      try {
        ...
        const [oldResolvedFragments, newResolvedFragments] = await Promise.all([
          this._resolveAppFragments(oldAppName, state),
          this._resolveAppFragments(newAppName, state),
        ]);

        if (!newResolvedFragments.every(Boolean)) {
          this._externalLink(source);
          return;
        }

        const oldFragmentNames = oldResolvedFragments
          .filter(Boolean)
          .map(({ name }) => name);

        const newFragmentNames = newResolvedFragments
          .filter(Boolean)
          .map(({ name }) => name);
    ...
    init = async (): Promise<void> => {
      ...
      try {
        ...
        const hydratePromises = resolvedFragments
          .filter(Boolean)
          .map(this._hydrate(state));
