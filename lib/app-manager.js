...
export default function initAppManager(w: Window) {
  return class AppManager implements AppManagerInterface {
    ...
    _mountedLifecycles: { [slotName: SlotNameType]: LifecycleType } = {};
    ...
    _updatePage = async (eventTitle: mixed, path: string, search: string): Promise<void> => {
      ...
      const sendErrors = async (err, index) => {
        const sendErrorsPromises = Object.keys(this._mountedLifecycles).map(async (slotName, errorIndex) => index !== errorIndex && this._mountedLifecycles[slotName].next(err));
        return Promise.all(sendErrorsPromises);
      };

      const unmountPromises = unmount.map(async (slotName, index) => {
        try {
          return await this._mountedLifecycles[slotName].next(state);
        } catch (err) {
          sendErrors(err, index);
          return null;
        }
      });
      ...
      render.forEach((slotName) => {
        this._mountedLifecycles[slotName] = this._scriptLifecycle(newAppFragments[slotName], slotName);
        this._mountedLifecycles[slotName].next(state);
      });

      const loadScriptPromises = render.map(async (slotName, index) => {
        try {
          return await this._mountedLifecycles[slotName].next(state);
        } catch (err) {
          sendErrors(err, index);
      ...
      const onStateChangePromises = onStateChange.map(async (slotName, index) => {
        try {
          return await this._mountedLifecycles[slotName].next(state);
        } catch (err) {
          sendErrors(err, index);
      ...
      const renderScriptPromises = render.map(async (slotName, index) => {
        try {
          return await this._mountedLifecycles[slotName].next(state);
        } catch (err) {
          sendErrors(err, index);
    ...
    async* _yieldErrors(fragmentName: FragmentNameType, state: StateType): AsyncGenerator<*, *, *> {
      ...
      while (value instanceof AppManagerError) {
        /* eslint-disable no-await-in-loop */

        try {
          await this._onUpdateStatus(fragmentName, state, this.constructor._getErrorStatusDetails(value));
        } catch (err) {
    ...
    async* _scriptLifecycle(fragmentName: FragmentNameType, slotName: SlotNameType, hydrate: boolean = true): LifecycleType {
      ...
      await this._getFragmentScript(fragmentName, getScriptState);
      await this._onUpdateStatus(fragmentName, getScriptState, defaultStatusDetails);

      const mountState = yield* this._yieldErrors(fragmentName, getScriptState);
    ...
    _setupPage = async (path: string, search: string): Promise<void> => {
      ...
      pageSlots.forEach((slotName) => {
        this._mountedLifecycles[slotName] = this._scriptLifecycle(pageFragments[slotName], slotName);
        this._mountedLifecycles[slotName].next(state);
      });
      ...
      const loadPageScripts = pageSlots.map(async (slotName, index) => {
        try {
          return await this._mountedLifecycles[slotName].next(state);
        } catch (err) {
          await sendErrors(err, index);
          return null;
        }
      });
      ...
      const hydratePageFragments = pageSlots.map(async (slotName, index) => {
        try {
          return await this._mountedLifecycles[slotName].next(state);
        } catch (err) {
          await sendErrors(err, index);
