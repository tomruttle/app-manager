...
export default function initAppManager(w: Window) {
  return class AppManager implements AppManagerInterface {
    ...
    _updatePage = async (eventTitle: mixed, path: string, search: string): Promise<?AppNameType> => {
      ...
      const render = newAppSlots.filter((slotName) => {
      ...
      const renderPromises = render.map(async (slotName) => {
        ...
        const pageFragment = newAppFragments[slotName];

        if (pageFragment) {
          this._mountedLifecycles[slotName] = { lifecycle: this._scriptLifecycle(pageFragment, slotName, false), done: false };
      ...
      await Promise.all([...onStateChange, ...render].map(nextAction));
    ...
    _setupPage = async (path: string, search: string): Promise<void> => {
      ...
      const loadPageScripts = pageSlots.map(async (slotName) => {
        const pageFragment = pageFragments[slotName];

        if (pageFragment) {
          this._mountedLifecycles[slotName] = { lifecycle: this._scriptLifecycle(pageFragment, slotName), done: false };
        } else {
          delete this._mountedLifecycles[slotName];
        }
