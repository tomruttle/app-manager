...
import type { PageScriptType, GuestAppType, AnalyticsType, OptionsType, ConfigType } from './index';

import decorateHistory from './utils/history-events';
...
export default function initAppManager(w: WindowType, html5History: History) {
  const history = decorateHistory(w, html5History);
  ...
  return class AppManager {
    ...
    constructor(config: ConfigType, analytics: AnalyticsType, options: OptionsType = {}) {
      ...
      if (!w.document || !w.document.location) {
        throw new Error('Invalid document object. Aborting.');
      }
    ...
    _getManagedSlotScripts = (appName: string): ?{ [appSlot: string]: string } => {
      ...
      if (!app || !Array.isArray(app.display)) {
        this._analytics.error({ eventTitle: `${this._analytics.namespace}.get_managed_slot_scripts.invalid_app`, id: appName });
        return null;
      }

      return app.display.reduce((scripts, displayScriptName) => {
        const guestAppScript = this._scripts[displayScriptName];

        if (!guestAppScript || !guestAppScript.managed) {
          return scripts;
        }

        if (!Array.isArray(guestAppScript.slots)) {
          this._analytics.error({ eventTitle: `${this._analytics.namespace}.get_managed_slot_scripts.invalid_script`, id: appName });
          return scripts;
        }
