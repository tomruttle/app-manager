...
import { DEFAULT_TIMEOUT, DOM_CHECK_INTERVAL, statuses, eventTitles } from './constants';
...
export default function initAppManager(w: Window) {
  return class AppManager {
    ...
    static async _timeout(timeout: number = DEFAULT_TIMEOUT): Promise<void> {
      return new Promise((resolve) => setTimeout(resolve, timeout));
    }

    static async _retry<T>(iteratee: (...Array<mixed>) => ?T, timeout: number = DEFAULT_TIMEOUT, interval: number): Promise<?T> {
    ...
    _getPathParams = (appName: AppNameType): ?Object => {
      const path = this.constructor._getBrowserPath();

      if (!path || typeof path !== 'string') {
        this._onError('get_params_from_path.no_path');
    ...
    _getElementFromDom = async (querySelector: string): Promise<?Element> => {
      const element = this.constructor._querySelector(querySelector);

      if (element) {
        return element;
      }

      return this.constructor._retry(
    ...
    _getSlotElement = async (slot: SlotType): Promise<?Element> => {
      ...
      const element = await this._getElementFromDom(querySelector);
    ...
    _hydrate = (appName: AppNameType) => async (resolvedFragment: ResolvedFragmentType): Promise<boolean> => {
      ...
      try {
        if (script.version === 3) {
          ...
          await script.hydrate(slotElement, { currentApp: app });
        } else if (typeof script.render === 'function') {
          await script.render(slotElement, { currentApp: app, eventTitle: eventTitles.HISTORY_REPLACE_STATE });
        }
    ...
    _unmount = (appName: AppNameType, eventTitle: EventTitleType) => async (resolvedFragment: ResolvedFragmentType): Promise<?boolean> => {
      ...
      try {
        ...
        if (script.version === 3) {
          ...
          await script.unmount(slotElement, { currentApp: app, eventTitle });
        }
    ...
    _mount = (appName: AppNameType, eventTitle: EventTitleType) => async (resolvedFragment: ResolvedFragmentType): Promise<boolean> => {
      ...
      try {
        ...
        if (script.version === 3) {
          ...
          await script.render(slotElement, { currentApp: app, eventTitle });
        }
    ...
    _onStateChange = (appName: AppNameType, eventTitle: EventTitleType) => async (resolvedFragment: ResolvedFragmentType): Promise<boolean> => {
      ...
      try {
        ...
        if (script.version === 3) {
          ...
          await script.onStateChange({ currentApp: app, eventTitle });
        }
    ...
    _onUpdateStatus = (appName: AppNameType, status: StatusType) => async (resolvedFragment: ResolvedFragmentType): Promise<boolean> => {
      ...
      try {
        ...
        if (script.version === 4) {
          ...
          await script.onUpdateStatus({ appStatus: status, currentApp: app });
        }
