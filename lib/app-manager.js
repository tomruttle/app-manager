...
export default function initAppManager(w: Window) {
  return class AppManager implements AppManagerInterface {
    ...
    async* _scriptLifecycle(initialState: StateType, fragmentName: FragmentNameType, slotName: SlotNameType, hydrate: boolean): LifecycleType {
      ...
      await this._onUpdateStatus(fragmentName, initialState, loadingStatusDetails);
      const script = await this._getFragmentScript(fragmentName, initialState);
      await this._onUpdateStatus(fragmentName, initialState, defaultStatusDetails);

      yield* this._yieldErrors(fragmentName, initialState);
      ...
      return yield* this._yieldErrors(fragmentName, unmountState);
    }
