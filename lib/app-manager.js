...
export default function initAppManager(w: Window) {
  return class AppManager {
    ...
    _queuedAction: ?{ path: string, eventTitle: EventTitleType } = null;
    ...
    _updatePage = async (eventTitle: string, path: string): Promise<void> => {
      ...
      try {
        ...
        if (this._queuedAction) {
          const { eventTitle: nextEventTitle, path: nextPath } = this._queuedAction;
          this._queuedAction = null;
          this._updatePage(nextEventTitle, nextPath);
        }
      } catch (err) {
    ...
    _handleStateChange = async (eventTitle: mixed): Promise<void> => {
      ...
      try {
        ...
        if (this._status === this.constructor.statuses.DEFAULT) {
          await this._updatePage(eventTitle, path);
        } else {
          this._queuedAction = { path, eventTitle };
        }
      } catch (err) {
        const amError = new AppManagerError(err, {
