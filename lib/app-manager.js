  ...
  return class AppManager implements AppManagerInterface {
    ...
    _updatePage = async (eventTitle: mixed, path: string, search: string): Promise<void> => {
      ...
      const handleError = async (slotName, err) => {
        ...
        const handleErrorPromises = Object.keys(this._mountedLifecycles)
          .map(async (errSlot) => !this._mountedLifecycles[errSlot].done && this._mountedLifecycles[errSlot].lifecycle.throw(err));
      ...
      const loadPageScriptsPromises = render.map(async (slotName) => {
        try {
          const { value } = await this._mountedLifecycles[slotName].lifecycle.next(state);
          return value;
        } catch (err) {
          await handleError(err);
          return true;
      ...
      const pageScripts = await Promise.all(loadPageScriptsPromises);
    ...
    _setupPage = async (path: string, search: string): Promise<void> => {
      ...
      const handleError = async (slotName, err) => {
        ...
        const handleErrorPromises = pageSlots
          .map(async (errSlot) => !this._mountedLifecycles[errSlot].done && this._mountedLifecycles[errSlot].lifecycle.throw(err));
...
//         return value;
      ...
      const pageScripts = await Promise.all(loadPageScripts);
