...
import initLoadWrappedScript from './utils/script';
...
export default function appManager(config: ConfigType, options: OptionsType) {
  ...
  const mountedLifecycles: { [slotName: SlotNameType]: LifecycleType } = new Map();
  const cachedScripts: { [fragmentName: FragmentNameType]: LoadedScriptType } = new Map();

  async function getFragmentScript(fragmentName: FragmentNameType, loadScript: LoadScriptType): Promise<LoadedScriptType> {
    if (cachedScripts.has(fragmentName)) {
      return cachedScripts.get(fragmentName);
    }
    ...
    try {
      const script = await loadScript(state);

      cachedScripts.set(fragmentName, script);
      return script;
    } catch (err) {
  ...
  const loadWrappedScript = initLoadWrappedScript(fragments, getFragmentScript, options);
  ...
  async function* createLifecycle(slotName: SlotNameType, fragmentName: FragmentNameType, onError: OnErrorCallbackType, initialRender: boolean): LifecycleType {
    const script = await loadWrappedScript(slotName, fragmentName);
  ...
  async function stateChanger(browserState: BrowserStateType, onRecoverableError?: OnErrorCallbackType): Promise<boolean> {
    ...
    const updatePromises = () => slotsToUpdate.map(async (slotName) => {
      try {
        if (mountedLifecycles.has(slotName)) {
          const { value } = await mountedLifecycles.get(slotName).next(false);
          return value;
        }
    ...
    const unmountPromises = () => slotsToUnmount.map(async (slotName) => {
      try {
        if (mountedLifecycles.has(slotName)) {
          const { value } = await mountedLifecycles.get(slotName).next(true);
          mountedLifecycles.delete(slotName);
          return value;
        }
