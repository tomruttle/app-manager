...
export type GetSlotElementType = (container: Element, slotName: SlotNameType, fragmentName: FragmentNameType) => Promise<?Element>;
...
export default function initGetElement(slots: SlotsMapType, getElement: GetElementType, importTimeout: number): GetSlotElementType {
  return async function getSlotElement(container, slotName, fragmentName) {
    const errData = {
    ...
    const element = await Promise.race([getElement(container, querySelector), delay(importTimeout)]);
