...
export default function appManager(config: ConfigType, options: OptionsType = {}) {
  ...
  const getRouteNameFromResource = options.getRouteNameFromResource || defaultGetRouteNameFromResource(routes);
  const updateState = initUpdateState(routes, getRouteNameFromResource, options.getAdditionalState);
        ...
        try {
          ...
          if (fragmentName) {
            ...
            if (typeof fragment.getMarkup === 'function') {
              markup = await fragment.getMarkup(slot.querySelector, state, ...getMarkupArgs);
            }
          }
        } catch (err) {
          ...
          if (typeof getErrorMarkup === 'function') {
            markup = await getErrorMarkup(state);
          }
        }
