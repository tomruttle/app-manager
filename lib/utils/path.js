...
export default function getPathHelpers(appPathsMap: { [appName: AppNameType]: { appPath?: string, appPaths?: Array<string> } }) {
  ...
  function evalPathParams(appName: string, path: string): ?ParamsType {
    return pathCheckers[appName].reduce((thing, checker, index) => {
      if (thing) {
        return thing;
      }
      ...
      const params = match
        ...
        .reduce((acc, token, idx) => Object.assign({}, acc, { [pathParams[appName][index][idx]]: token }), {});
  ...
  return {
    getAppNameFromResource(resource: string): ?AppNameType {
    ...
    getAdditionalState(appName: AppNameType, resource: string): { params: ParamsType, query: ParamsType } {
