...
export default class WindowStub {
  ...
  constructor(initialHistoryState: Array<StateEntry> = [{ data: {}, title: null, hash: '/' }]) {
    ...
    this._position = initialHistoryState.length === 0 ? 0 : initialHistoryState.length - 1;
    this._state = [...initialHistoryState];
    this._updateLocation(initialPath);
  }
  ...
  history = {
    ...
    back: () => {
      ...
      this._updateLocation(newState.hash);
