...
export function setNames<M: { [key: string]: mixed }>(map: { [key: string]: M }): { [key: string]: M & { name: string } } {
  const errData = {
    source: 'set_names',
    fragment: null,
    slot: null,
  ...
  if (typeof map !== 'object') {
    throw new AppManagerError('Invalid config: map is not an object.', Object.assign({}, errData, {
      level: levels.ERROR,
      code: 'invalid_map',
      recoverable: false,
    }));
  ...
  return Object.keys(map).reduce((acc, key) => {
    if (typeof map[key] !== 'object') {
      throw new AppManagerError(`Invalid config for ${key}: not an object.`, Object.assign({}, errData, {
        level: levels.ERROR,
