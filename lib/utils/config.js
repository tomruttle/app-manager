...
export function defaultGetAppNameFromUrl(apps: { [appName: AppNameType]: BaseAppType }) {
...
function generateRandomChars() {
  return `000${generateRandomNumber().toString(36)}`.slice(-3);
}

export function generateUID() {
  return `${generateRandomChars()}${generateRandomChars()}`;
}
