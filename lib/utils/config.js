...
export function defaultGetRouteName(routes: { [routeName: RouteNameType]: RouteType }): GetRouteNameType {
  ...
  return function getRouteNameFromResource(state) {
    const [path] = state.resource.split('?');
