...
export default function initScriptFunctions(fragments: FragmentsMapType, options: OptionsType, updateState: UpdateStateType) {
  ...
  async function loadScript(fragment, state) {
    const errData = {
      source: 'load_script',
  ...
  return async function getWrappedScript(slotName: SlotNameType, fragmentName: FragmentNameType): Promise<?ScriptType> {
    ...
    const script = await Promise.race([loadScript(fragment, state), delay(options.importTimeout)]);
