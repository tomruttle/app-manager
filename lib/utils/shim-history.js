...
import type { EventsType, StateChangeType } from '../index';
...
export default function iniHistoryCallback(w: Window) {
  ...
  return function historyCallback(events: EventsType, callback: any) {
    ...
    let workingState: ?BrowserStateType;

    async function stateChanger(eventTitle?: mixed) {
      try {
        const oldState = workingState;
        ...
        const proceed = await callback(newState, oldState);
