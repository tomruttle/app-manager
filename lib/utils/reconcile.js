...
export default function initReconcile(config: ConfigType) {
  ...
  return function reconcile(newAppName: ?AppNameType, oldAppName: ?AppNameType): PageDiffType {
    const errData = {
      slot: null,
      fragment: null,
      source: 'reconcile',
    ...
    const newAppFragments = getPageFragments(newAppName);
    ...
    const oldAppFragments = getPageFragments(oldAppName);
