...
export default function iniHistoryCallback(w: any) {
  ...
  function shimHistory(events: EventsType, callback: (eventTitle: string) => void) {
    ...
    w.history.pushState = function hcPushState(...args) {
    ...
    w.history.replaceState = function hcReplaceState(...args) {
    ...
    w.onpopstate = function hcOnpopstate(...args) {
    ...
    w.onbeforeunload = function hcOnbeforeunload(...args) {
  ...
  return function historyCallback(callback: HistoryCallbackType, events: EventsType, options?: OptionsType) {
    ...
    async function stateChanger(eventTitle: string) {
      try {
        ...
        const rootElement = parentElement || w.document.body;

        await callback(rootElement, {
          rootState: true,
