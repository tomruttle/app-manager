...
import type { ScriptType, EventsType } from './index';
...
export default function iniHistoryCallback(w: Window) {
  ...
  return function historyCallback(script: ScriptType, events: EventsType) {
    ...
    async function stateChanger(eventTitle: string) {
      try {
        ...
        const proceed = eventTitle === eventTitles.INITIALISE
          ? await script.render(w.document, state)
          : await script.onStateChange(w.document, state);
